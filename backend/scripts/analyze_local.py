#!/usr/bin/env python3
"""Standalone driver that reproduces the Motus *side-view* analysis path.

Milestone 1 goal: prove the extracted biomechanics core works on its own --
produce a numeric technique result for RUNNING (side view) and CYCLING
(side view) from a local video file. NO API, DB, R2, LLM, or video overlay.

The call order below mirrors ``VideoAnalysisPipeline.process_video`` in Motus
(the proven side-view path only):

    build_detector -> extract frames (CLAHE + detect) -> stabilize_landmarks
    -> determine_locked_camera_side -> landmark quality
    -> analyzer.analyze_frame loop -> finalize_camera_side
    -> run_advanced_biomechanics -> compute_summary / detect_issues /
       compute_angle_statistics -> quality gate -> score_analysis

Helper functions (_compute_landmark_quality, _detect_skeleton_jumps,
_build_quality_warnings, _compute_unknown_gait_phase_pct_run, the frame
extractor) are copied verbatim from Motus ``pipeline.py`` -- copied, not
rewritten, to keep the numeric behaviour identical.

Usage:
    python analyze_local.py <video_path> <run|bike> [--position road_hoods]
"""
...
from app.services.video_analysis.pipeline import SPORT_SAMPLE_RATES  # noqa: E402
from app.services.video_analysis.detectors import PoseDetector, build_detector  # noqa: E402
from app.services.video_analysis.biomechanics.advanced_pipeline import (  # noqa: E402
    run_advanced_biomechanics,
)
from app.services.video_analysis.biomechanics.cycling_analyzer import (  # noqa: E402
    CyclingAnalyzer,
    determine_locked_camera_side,
)
from app.services.video_analysis.biomechanics.landmark_stabilizer import (  # noqa: E402
    stabilize_landmarks,
)
from app.services.video_analysis.biomechanics.quality_gate import (  # noqa: E402
    evaluate_quality_gate,
)
from app.services.video_analysis.biomechanics.running_analyzer import (  # noqa: E402
    RunningAnalyzer,
)
from app.services.video_analysis.biomechanics.technique_scorer import (  # noqa: E402
    score_analysis,
)
